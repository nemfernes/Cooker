...
class DishDetailViewController: UIViewController {
    ...
    @IBOutlet weak var editButton: UIButton!  {
        didSet {
            editButton.setTitle(LS.Common.Strings.editRecept.localized, for: .normal)
            editButton.setTitleColor(.white, for: .normal)
            editButton.titleLabel?.font = .sfSemiBold16
            editButton.backgroundColor = UIColor.asset(.color_1E1E1E)
    ...
    init(dish: Dish) {
        self.dish = dish
        super.init(nibName: "DishDetailViewController", bundle: nil)
    }
    ...
    @objc private func editTapped(_ sender: UIButton) {
        self.router?.goToEdit(dishType: DishType(rawValue: dish.type) ?? .mainCourses, dish: dish)
    }
}
